...
fn print_with_boundaries(content: &str, origin: Option<&str>, opts: &OutputOptions) {
    ...
    if opts.content_boundaries {
        ...
        println!(
            "--- VEIL_PAGE_CONTENT nonce={} origin={} ---",
            nonce, origin_str
        );
        println!("{}", content);
        println!("--- END_VEIL_PAGE_CONTENT nonce={} ---", nonce);
    } else {
        println!("{}", content);
    }
}
...
pub fn print_response_with_opts(resp: &Response, action: Option<&str>, opts: &OutputOptions) {
    ...
    if let Some(data) = &resp.data {
            ...
            .unwrap_or(false)
        {
            ...
            println!("  Run: veil confirm {}", cid);
            println!("  Or:  veil deny {}", cid);
            return;
        }
        if data
            .get("confirmed")
...
Examples:
  veil open example.com
  veil open https://github.com
  veil open localhost:3000
  veil open api.example.com --headers '{"Authorization": "Bearer token"}'
    # ^ Headers only sent to api.example.com, not other domains
"##
        }
        "back" => {
            r##"
veil back - Navigate back in history
...
"##
        ...
        "forward" => {
            r##"
veil forward - Navigate forward in history
...
"##
        ...
        "reload" => {
            r##"
veil reload - Reload the current page
...
"##
        ...
        "click" => {
            r##"
veil click - Click an element

Usage: veil click <selector> [--new-tab]
...
Examples:
  veil click "#submit-button"
  veil click @e1
  veil click "button.primary"
  veil click "//button[@type='submit']"
  veil click @e3 --new-tab
"##
        }
        "dblclick" => {
            r##"
veil dblclick - Double-click an element

Usage: veil dblclick <selector>
...
Examples:
  veil dblclick "#editable-text"
  veil dblclick @e5
"##
        }
        "fill" => {
            r##"
veil fill - Clear and fill an input field

Usage: veil fill <selector> <text>
...
Examples:
  veil fill "#email" "user@example.com"
  veil fill @e3 "Hello World"
  veil fill "input[name='search']" "query"
"##
        }
        "type" => {
            r##"
veil type - Type text into an element

Usage: veil type <selector> <text>
...
Examples:
  veil type "#search" "hello"
  veil type @e2 "additional text"

See Also:
  For typing into contenteditable editors (Lexical, ProseMirror, etc.)
  without a selector, use 'keyboard type' instead:
    veil keyboard type "# My Heading"
"##
        }
        "hover" => {
            r##"
veil hover - Hover over an element

Usage: veil hover <selector>
...
Examples:
  veil hover "#dropdown-trigger"
  veil hover @e4
"##
        }
        "focus" => {
            r##"
veil focus - Focus an element

Usage: veil focus <selector>
...
Examples:
  veil focus "#input-field"
  veil focus @e2
"##
        }
        "check" => {
            r##"
veil check - Check a checkbox

Usage: veil check <selector>
...
Examples:
  veil check "#terms-checkbox"
  veil check @e7
"##
        }
        "uncheck" => {
            r##"
veil uncheck - Uncheck a checkbox

Usage: veil uncheck <selector>
...
Examples:
  veil uncheck "#newsletter-opt-in"
  veil uncheck @e8
"##
        }
        "select" => {
            r##"
veil select - Select a dropdown option

Usage: veil select <selector> <value...>
...
Examples:
  veil select "#country" "US"
  veil select @e5 "option2"
  veil select "#menu" "opt1" "opt2" "opt3"
"##
        }
        "drag" => {
            r##"
veil drag - Drag and drop

Usage: veil drag <source> <target>
...
Examples:
  veil drag "#draggable" "#drop-zone"
  veil drag @e1 @e2
"##
        }
        "upload" => {
            r##"
veil upload - Upload files

Usage: veil upload <selector> <files...>
...
Examples:
  veil upload "#file-input" ./document.pdf
  veil upload @e3 ./image1.png ./image2.png
"##
        }
        "download" => {
            r##"
veil download - Download a file by clicking an element

Usage: veil download <selector> <path>
...
Examples:
  veil download "#download-btn" ./file.pdf
  veil download @e5 ./report.xlsx
  veil download "a[href$='.zip']" ./archive.zip
"##
        }
        ...
        "press" | "key" => {
            r##"
veil press - Press a key or key combination

Usage: veil press <key>
...
Examples:
  veil press Enter
  veil press Tab
  veil press Control+a
  veil press Control+Shift+s
  veil press Escape
"##
        }
        "keydown" => {
            r##"
veil keydown - Press a key down (without release)

Usage: veil keydown <key>
...
Examples:
  veil keydown Shift
  veil keydown Control
"##
        }
        "keyup" => {
            r##"
veil keyup - Release a key

Usage: veil keyup <key>
...
"##
        ...
        "keyboard" => {
            r##"
veil keyboard - Raw keyboard input (no selector needed)

Usage: veil keyboard <subcommand> <text>
...
Examples:
  veil keyboard type "Hello, World!"
  veil keyboard type "# My Heading"
  veil keyboard inserttext "pasted content"

Use Cases:
  # Type into a Lexical/ProseMirror contenteditable editor:
  veil click "[contenteditable]"
  veil keyboard type "# My Heading"
  veil press Enter
  veil keyboard type "Some paragraph text"
"##
        }
        ...
        "scroll" => {
            r##"
veil scroll - Scroll the page

Usage: veil scroll [direction] [amount] [options]
...
Examples:
  veil scroll
  veil scroll down 500
  veil scroll up 200
  veil scroll left 100
  veil scroll down 500 --selector "div.scroll-container"
"##
        }
        "scrollintoview" | "scrollinto" => {
            r##"
veil scrollintoview - Scroll element into view

Usage: veil scrollintoview <selector>
...
Examples:
  veil scrollintoview "#footer"
  veil scrollintoview @e15
"##
        }
        ...
        "wait" => {
            r##"
veil wait - Wait for condition

Usage: veil wait <selector|ms|option>
...
Examples:
  veil wait "#loading-spinner"
  veil wait 2000
  veil wait --url "**/dashboard"
  veil wait --load networkidle
  veil wait --fn "window.appReady === true"
  veil wait --text "Welcome back"
  veil wait --download ./file.pdf
  veil wait --download ./report.xlsx --timeout 30000
  veil wait --fn "!document.body.innerText.includes('Loading...')"
"##
        }
        ...
        "screenshot" => {
            r##"
veil screenshot - Take a screenshot

Usage: veil screenshot [selector] [path]
...
Examples:
  veil screenshot
  veil screenshot ./screenshot.png
  veil screenshot --full ./full-page.png
  veil screenshot --annotate              # Labeled screenshot + legend
  veil screenshot --annotate ./page.png   # Save annotated screenshot
  veil screenshot --annotate --json       # JSON output with annotations
  veil screenshot --screenshot-dir ./shots # Save to custom directory
  veil screenshot --screenshot-format jpeg --screenshot-quality 80
"##
        }
        "pdf" => {
            r##"
veil pdf - Save page as PDF
...
"##
        ...
        "snapshot" => {
            r##"
veil snapshot - Get accessibility tree snapshot

Usage: veil snapshot [options]
...
Examples:
  veil snapshot
  veil snapshot -i
  veil snapshot --compact --depth 5
  veil snapshot -s "#main-content"
"##
        }
        ...
        "eval" => {
            r##"
veil eval - Execute JavaScript

Usage: veil eval [options] <script>
...
Examples:
  ...
  veil eval "window.location.href"
  veil eval "document.querySelectorAll('a').length"
  veil eval -b "ZG9jdW1lbnQudGl0bGU="

  # Read from stdin with heredoc
  cat <<'EOF' | veil eval --stdin
...
"##
        ...
        "close" | "quit" | "exit" => {
            r##"
veil close - Close the browser

Usage: veil close [options]
...
"##
        ...
        "inspect" => {
            r##"
veil inspect - Open Chrome DevTools for the active page
...
existing CDP connection, so both DevTools and veil commands work
simultaneously.
...
Examples:
  veil open example.com
  veil inspect          # opens DevTools in your browser
  veil click "Submit"   # commands still work while DevTools is open
"##
        }
        ...
        "get" => {
            r##"
veil get - Retrieve information from elements or page

Usage: veil get <subcommand> [args]
...
Examples:
  veil get text @e1
  veil get html "#content"
  veil get value "#email-input"
  veil get attr "#link" href
  veil get title
  veil get url
  veil get count "li.item"
  veil get box "#header"
  veil get styles "button"
  veil get styles @e1
"##
        }
        ...
        "is" => {
            r##"
veil is - Check element state

Usage: veil is <subcommand> <selector>
...
Examples:
  veil is visible "#modal"
  veil is enabled "#submit-btn"
  veil is checked "#agree-checkbox"
"##
        }
        ...
        "find" => {
            r##"
veil find - Find and interact with elements by locator

Usage: veil find <locator> <value> [action] [text]
...
Examples:
  veil find role button click --name Submit
  veil find text "Sign In" click
  veil find label "Email" fill "user@example.com"
  veil find placeholder "Search..." type "query"
  veil find testid "login-form" click
  veil find first "li.item" click
  veil find nth 2 ".card" hover
"##
        }
        ...
        "mouse" => {
            r##"
veil mouse - Low-level mouse operations

Usage: veil mouse <subcommand> [args]
...
Examples:
  veil mouse move 100 200
  veil mouse down
  veil mouse up
  veil mouse down right
  veil mouse wheel 100
  veil mouse wheel -50 0
"##
        }
        ...
        "set" => {
            r##"
veil set - Configure browser settings

Usage: veil set <setting> [args]
...
Examples:
  veil set viewport 1920 1080
  veil set viewport 1920 1080 2    # 2x retina
  veil set device "iPhone 12"
  veil set geo 37.7749 -122.4194
  veil set offline on
  veil set headers '{"X-Custom": "value"}'
  veil set credentials admin secret123
  veil set media dark
  veil set media light reduced-motion
"##
        }
        ...
        "network" => {
            r##"
veil network - Network interception and monitoring

Usage: veil network <subcommand> [args]
...
Examples:
  veil network route "**/api/*" --abort
  veil network route "**/data.json" --body '{"mock": true}'
  veil network unroute
  veil network requests
  veil network requests --filter "api"
  veil network requests --type xhr,fetch
  veil network requests --method POST --status 2xx
  veil network requests --clear
  veil network request 1234.5
  veil network har start
  veil network har stop ./capture.har
"##
        }
        ...
        "storage" => {
            r##"
veil storage - Manage web storage

Usage: veil storage <type> [operation] [key] [value]
...
Examples:
  veil storage local
  veil storage local get authToken
  veil storage local set theme "dark"
  veil storage local clear
  veil storage session get userId
"##
        }
        ...
        "cookies" => {
            r##"
veil cookies - Manage browser cookies

Usage: veil cookies [operation] [args]
...
Examples:
  ...
  veil cookies set session_id "abc123"

  # Set cookie for a URL before loading it (useful for authentication)
  veil cookies set session_id "abc123" --url https://app.example.com

  # Set secure, httpOnly cookie with domain and path
  veil cookies set auth_token "xyz789" --domain example.com --path /api --httpOnly --secure

  # Set cookie with SameSite policy
  veil cookies set tracking_consent "yes" --sameSite Strict

  # Set cookie with expiration (Unix timestamp)
  veil cookies set temp_token "temp123" --expires 1735689600
...
"##
        ...
        "tab" => {
            r##"
veil tab - Manage browser tabs

Usage: veil tab [operation] [args]
...
Examples:
  ...
  veil tab list
  veil tab new
  veil tab new https://example.com
  veil tab 2
  veil tab close
  veil tab close 1
"##
        ...
        "window" => {
            r##"
veil window - Manage browser windows

Usage: veil window <operation>
...
"##
        ...
        "frame" => {
            r##"
veil frame - Switch frame context

Usage: veil frame <selector|main>
...
Examples:
  veil frame "#embed-iframe"
  veil frame "iframe[name='content']"
  veil frame main
"##
        }
        ...
        "auth" => {
            r##"
veil auth - Manage authentication profiles

Usage: veil auth <subcommand> [args]
...
Examples:
  echo "pass" | veil auth save github --url https://github.com/login --username user --password-stdin
  veil auth save github --url https://github.com/login --username user --password pass
  veil auth login github
  veil auth list
  veil auth show github
  veil auth delete github
"##
        }
        ...
        "confirm" | "deny" => {
            r##"
veil confirm/deny - Approve or deny pending actions

Usage:
  veil confirm <confirmation-id>
  veil deny <confirmation-id>
...
Examples:
  veil confirm c_8f3a1234
  veil deny c_8f3a1234
"##
        }
        ...
        "dialog" => {
            r##"
veil dialog - Handle browser dialogs

Usage: veil dialog <accept|dismiss|status> [text]
...
"##
        ...
        "trace" => {
            r##"
veil trace - Record execution trace

Usage: veil trace <operation> [path]
...
Examples:
  veil trace start
  veil trace start ./my-trace
  veil trace stop
  veil trace stop ./debug-trace.zip
"##
        }
        ...
        "profiler" => {
            r##"
veil profiler - Record Chrome DevTools performance profile

Usage: veil profiler <operation> [options]
...
Examples:
  # Basic profiling
  veil profiler start
  veil navigate https://example.com
  veil click "#button"
  veil profiler stop ./trace.json

  # With custom categories
  veil profiler start --categories "devtools.timeline,v8.execute,blink.user_timing"
  veil profiler stop ./custom-trace.json
...
"##
        ...
        "record" => {
            r##"
veil record - Record browser session to video

Usage: veil record start <path.webm> [url]
       veil record stop
       veil record restart <path.webm> [url]
...
Examples:
  ...
  veil open https://app.example.com/dashboard
  veil snapshot -i            # Explore and plan
  veil record start ./demo.webm
  veil click @e3              # Execute planned actions
  veil record stop

  # Or specify a different URL
  veil record start ./demo.webm https://example.com

  # Restart recording with a new file (stops previous, starts new)
  veil record restart ./take2.webm
"##
        }
        ...
        "console" => {
            r##"
veil console - View console logs

Usage: veil console [--clear]
...
"##
        ...
        "errors" => {
            r##"
veil errors - View page errors

Usage: veil errors [--clear]
...
"##
        ...
        "highlight" => {
            r##"
veil highlight - Highlight an element

Usage: veil highlight <selector>
...
"##
        ...
        "clipboard" => {
            r##"
veil clipboard - Read and write clipboard

Usage: veil clipboard <operation> [text]
...
Examples:
  veil clipboard read
  veil clipboard write "Hello, World!"
  veil clipboard copy
  veil clipboard paste
"##
        }
        ...
        "state" => {
            r##"
veil state - Manage browser state

Usage: veil state <operation> [args]
...
Automatic State Persistence:
  ...
  veil --session-name myapp open https://example.com
  Or set VEIL_SESSION_NAME environment variable.

State Encryption:
  Set VEIL_ENCRYPTION_KEY (64-char hex) for AES-256-GCM encryption.
...
Examples:
  veil state save ./auth-state.json
  veil state load ./auth-state.json
  veil state list
  veil state show myapp-default.json
  veil state rename old-name new-name
  veil state clear --all
  veil state clean --older-than 7
"##
        }
        ...
        "session" => {
            r##"
veil session - Manage sessions

Usage: veil session [operation]
...
Environment:
  VEIL_SESSION    Default session name
...
Examples:
  veil session
  veil session list
  veil --session test open example.com
"##
        }
        ...
        "install" => {
            r##"
veil install - Install browser binaries

Usage: veil install [--with-deps]
...
"##
        ...
        "upgrade" => {
            r##"
veil upgrade - Upgrade to the latest version
...
"##
        ...
        "dashboard" => {
            r##"
veil dashboard - Observability dashboard

Usage: veil dashboard [start|stop|install] [options]
...
Subcommands:
  ...
  install              Download and install the dashboard to ~/.veil/dashboard/

Running 'veil dashboard' with no subcommand is equivalent to 'dashboard start'.
...
Examples:
  veil dashboard install
  veil dashboard start
  veil dashboard start --port 8080
  veil dashboard stop
"##
        }
        ...
        "connect" => {
            r##"
veil connect - Connect to browser via CDP

Usage: veil connect <port|url>
...
Examples:
  ...
  veil connect "ws://localhost:9222/devtools/browser/abc123"

  # Connect to remote browser service
  veil connect "wss://browser-service.example.com/cdp?token=xyz"
...
"##
        ...
        "stream" => {
            r##"
veil stream - Manage live WebSocket browser streaming

Usage:
  veil stream enable [--port <port>]
  veil stream disable
  veil stream status

Enables or disables the session-scoped WebSocket stream server without restarting
an already-running daemon. If --port is omitted, veil binds an
...
Notes:
  ...
  - Streaming is always enabled. Set VEIL_STREAM_PORT to bind to a
...
"##
        ...
        "tap" => {
            r##"
veil tap - Tap an element (touch gesture)

Usage: veil tap <selector>
...
Examples:
  veil tap "#submit-button"
  veil tap @e1
  veil -p ios tap "button:has-text('Sign In')"
"##
        }
        "swipe" => {
            r##"
veil swipe - Swipe gesture (iOS)

Usage: veil swipe <direction> [distance]
...
Examples:
  veil -p ios swipe up
  veil -p ios swipe down 500
  veil -p ios swipe left
"##
        }
        "device" => {
            r##"
veil device - Manage iOS simulators

Usage: veil device <subcommand>
...
Examples:
  veil device list
  veil -p ios device list
"##
        }

        "diff" => {
            r##"
veil diff - Compare page states
...
Snapshot Diff:
  ...
  Usage: veil diff snapshot [options]
...
Screenshot Diff:
  ...
  Usage: veil diff screenshot --baseline <file> [options]
...
URL Diff:
  ...
  Usage: veil diff url <url1> <url2> [options]
...
Examples:
  veil diff snapshot
  veil diff snapshot --baseline before.txt
  veil diff screenshot --baseline before.png
  veil diff screenshot --baseline before.png --output diff.png --threshold 0.2
  veil diff url https://staging.example.com https://prod.example.com
  veil diff url https://v1.example.com https://v2.example.com --screenshot
"##
        }

        "batch" => {
            r##"
veil batch - Execute multiple commands from stdin

Usage: echo '<json>' | veil batch [options]
...
Examples:
  echo '[["open", "https://example.com"], ["snapshot"]]' | veil batch
  echo '[["open", "https://example.com"], ["get", "title"]]' | veil batch --json
  veil batch --bail < commands.json
"##
        }
...
pub fn print_help() {
    println!(
        r#"
veil - fast browser automation CLI for AI agents

Usage: veil <command> [args] [options]
...
Get Info:  veil get <what> [selector]
  text, html, value, attr <name>, title, url, count, box, styles, cdp-url

Check State:  veil is <what> <selector>
  visible, enabled, checked

Find Elements:  veil find <locator> <value> <action> [text]
  role, text, label, placeholder, alt, title, testid, first, last, nth

Mouse:  veil mouse <action> [args]
  move <x> <y>, down [btn], up [btn], wheel <dy> [dx]

Browser Settings:  veil set <setting> [value]
  viewport <w> <h>, device <name>, geo <lat> <lng>
  offline [on|off], headers <json>, credentials <user> <pass>
  media [dark|light] [reduced-motion]

Network:  veil network <action>
...
Authentication:
  ...
  --auto-connect             Connect to a running Chrome to reuse its auth state
                             Tip: veil --auto-connect state save ./auth.json
  --headers <json>           HTTP headers scoped to URL's origin (e.g., Authorization bearer token)

Options:
  --session <name>           Isolated session (or VEIL_SESSION env)
  --executable-path <path>   Custom browser executable (or VEIL_EXECUTABLE_PATH)
  --extension <path>         Load browser extensions (repeatable)
  --args <args>              Browser launch args, comma or newline separated (or VEIL_ARGS)
                             e.g., --args "--no-sandbox,--disable-blink-features=AutomationControlled"
  --user-agent <ua>          Custom User-Agent (or VEIL_USER_AGENT)
  --proxy <server>           Proxy server URL (or VEIL_PROXY, HTTP_PROXY, HTTPS_PROXY, ALL_PROXY)
                             Supports authenticated proxies: --proxy "http://user:pass@127.0.0.1:7890"
  --proxy-bypass <hosts>     Bypass proxy for these hosts (or VEIL_PROXY_BYPASS, NO_PROXY)
  ...
  --screenshot-dir <path>    Default screenshot output directory (or VEIL_SCREENSHOT_DIR)
  --screenshot-quality <n>   JPEG quality 0-100; ignored for PNG (or VEIL_SCREENSHOT_QUALITY)
  --screenshot-format <fmt>  Screenshot format: png, jpeg (or VEIL_SCREENSHOT_FORMAT)
  --headed                   Show browser window (not headless) (or VEIL_HEADED env)
  --cdp <port>               Connect via CDP (Chrome DevTools Protocol)
  --color-scheme <scheme>    Color scheme: dark, light, no-preference (or VEIL_COLOR_SCHEME)
  --download-path <path>     Default download directory (or VEIL_DOWNLOAD_PATH)
  --content-boundaries       Wrap page output in boundary markers (or VEIL_CONTENT_BOUNDARIES)
  --max-output <chars>       Truncate page output to N chars (or VEIL_MAX_OUTPUT)
  --allowed-domains <list>   Restrict navigation domains (or VEIL_ALLOWED_DOMAINS)
  --action-policy <path>     Action policy JSON file (or VEIL_ACTION_POLICY)
  --confirm-actions <list>   Categories requiring confirmation (or VEIL_CONFIRM_ACTIONS)
  --confirm-interactive      Interactive confirmation prompts; auto-denies if stdin is not a TTY (or VEIL_CONFIRM_INTERACTIVE)
  --engine <name>            Browser engine: chrome (default), lightpanda (or VEIL_ENGINE)
  --config <path>            Use a custom config file (or VEIL_CONFIG env)
  --debug                    Debug output
  --version, -V              Show version

Configuration:
  veil looks for veil.json in these locations (lowest to highest priority):
    1. ~/.veil/config.json      User-level defaults
    2. ./veil.json              Project-level overrides
    3. Environment variables             Override config file values
    4. CLI flags                         Override everything

  Use --config <path> to load a specific config file instead of the defaults.
  If --config points to a missing or invalid file, veil exits with an error.
...
Environment:
  VEIL_CONFIG           Path to config file (or use --config)
  VEIL_SESSION          Session name (default: "default")
  VEIL_SESSION_NAME     Auto-save/restore state persistence name
  VEIL_ENCRYPTION_KEY   64-char hex key for AES-256-GCM state encryption
  VEIL_STATE_EXPIRE_DAYS Auto-delete states older than N days (default: 30)
  VEIL_EXECUTABLE_PATH  Custom browser executable path
  VEIL_EXTENSIONS       Comma-separated browser extension paths
  VEIL_HEADED           Show browser window (not headless)
  VEIL_JSON             JSON output
  VEIL_ANNOTATE         Annotated screenshot with numbered labels and legend
  VEIL_DEBUG            Debug output
  VEIL_IGNORE_HTTPS_ERRORS Ignore HTTPS certificate errors
  VEIL_PROVIDER         Browser provider (ios, browserbase, kernel, browseruse, browserless)
  VEIL_AUTO_CONNECT     Auto-discover and connect to running Chrome
  VEIL_ALLOW_FILE_ACCESS Allow file:// URLs to access local files
  VEIL_COLOR_SCHEME     Color scheme preference (dark, light, no-preference)
  VEIL_DOWNLOAD_PATH    Default download directory for browser downloads
  VEIL_DEFAULT_TIMEOUT  Default action timeout in ms (default: 25000)
  VEIL_SESSION_NAME     Auto-save/load state persistence name
  VEIL_STATE_EXPIRE_DAYS Auto-delete saved states older than N days (default: 30)
  VEIL_ENCRYPTION_KEY   64-char hex key for AES-256-GCM session encryption
  VEIL_STREAM_PORT      Override WebSocket streaming port (default: OS-assigned)
  VEIL_IDLE_TIMEOUT_MS  Auto-shutdown daemon after N ms of inactivity (disabled by default)
  VEIL_IOS_DEVICE       Default iOS device name
  VEIL_IOS_UDID         Default iOS device UDID
  VEIL_CONTENT_BOUNDARIES Wrap page output in boundary markers
  VEIL_MAX_OUTPUT       Max characters for page output
  VEIL_ALLOWED_DOMAINS  Comma-separated allowed domain patterns
  VEIL_ACTION_POLICY    Path to action policy JSON file
  VEIL_CONFIRM_ACTIONS  Action categories requiring confirmation
  VEIL_CONFIRM_INTERACTIVE Enable interactive confirmation prompts
  VEIL_ENGINE           Browser engine: chrome (default), lightpanda
  HTTP_PROXY / HTTPS_PROXY       Standard proxy env vars (fallback if VEIL_PROXY not set)
  ALL_PROXY                      SOCKS proxy (fallback for proxy)
  NO_PROXY                       Bypass proxy for hosts (fallback for proxy-bypass)
  VEIL_SCREENSHOT_DIR   Default screenshot output directory
  VEIL_SCREENSHOT_QUALITY JPEG quality 0-100
  VEIL_SCREENSHOT_FORMAT Screenshot format: png, jpeg

Install:
  npm install -g veil           # npm
  brew install veil             # Homebrew
  cargo install veil            # Cargo
  veil install                  # Download Chrome (first time)

Examples:
  veil open example.com
  veil snapshot -i              # Interactive elements only
  veil click @e2                # Click by ref from snapshot
  veil fill @e3 "test@example.com"
  veil find role button click --name Submit
  veil get text @e1
  veil screenshot --full
  veil screenshot --annotate    # Labeled screenshot for vision models
  veil wait --load networkidle  # Wait for slow pages to load
  veil --cdp 9222 snapshot      # Connect via CDP port
  veil --auto-connect snapshot  # Auto-discover running Chrome
  veil stream enable            # Start runtime streaming on an auto-selected port
  veil stream status            # Inspect runtime streaming state
  veil --color-scheme dark open example.com  # Dark mode
  veil --profile ~/.myapp open example.com    # Persistent profile
  veil --session-name myapp open example.com  # Auto-save/restore state

Command Chaining:
  Chain commands with && in a single shell call (browser persists via daemon):

  veil open example.com && veil wait --load networkidle && veil snapshot -i
  veil fill @e1 "user@example.com" && veil fill @e2 "pass" && veil click @e3
  veil open example.com && veil wait --load networkidle && veil screenshot page.png

iOS Simulator (requires Xcode and Appium):
  veil -p ios open example.com                    # Use default iPhone
  veil -p ios --device "iPhone 15 Pro" open url   # Specific device
  veil -p ios device list                         # List simulators
  veil -p ios swipe up                            # Swipe gesture
  veil -p ios tap @e1                             # Touch element
"#
    );
}
...
pub fn print_version() {
    println!("veil {}", env!("CARGO_PKG_VERSION"));
}
