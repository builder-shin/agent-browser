...
/// Priority: VEIL_SOCKET_DIR > XDG_RUNTIME_DIR > ~/.veil > tmpdir
pub fn get_socket_dir() -> PathBuf {
    ...
    if let Ok(runtime_dir) = env::var("XDG_RUNTIME_DIR") {
        if !runtime_dir.is_empty() {
            return PathBuf::from(runtime_dir).join("veil");
        }
    }
...
fn apply_daemon_env(cmd: &mut Command, session: &str, opts: &DaemonOptions) {
    cmd.env("VEIL_DAEMON", "1")
        .env("VEIL_SESSION", session);

    if opts.headed {
        cmd.env("VEIL_HEADED", "1");
    }
    if opts.debug {
        cmd.env("VEIL_DEBUG", "1");
    }
    if let Some(path) = opts.executable_path {
        cmd.env("VEIL_EXECUTABLE_PATH", path);
    }
    if !opts.extensions.is_empty() {
        cmd.env("VEIL_EXTENSIONS", opts.extensions.join(","));
    }
    if let Some(a) = opts.args {
        cmd.env("VEIL_ARGS", a);
    }
    if let Some(ua) = opts.user_agent {
        cmd.env("VEIL_USER_AGENT", ua);
    }
    if let Some(p) = opts.proxy {
        cmd.env("VEIL_PROXY", p);
    }
    if let Some(pb) = opts.proxy_bypass {
        cmd.env("VEIL_PROXY_BYPASS", pb);
    }
    if let Some(pu) = opts.proxy_username {
        cmd.env("VEIL_PROXY_USERNAME", pu);
    }
    if let Some(pp) = opts.proxy_password {
        cmd.env("VEIL_PROXY_PASSWORD", pp);
    }
    if opts.ignore_https_errors {
        cmd.env("VEIL_IGNORE_HTTPS_ERRORS", "1");
    }
    if opts.allow_file_access {
        cmd.env("VEIL_ALLOW_FILE_ACCESS", "1");
    }
    if let Some(prof) = opts.profile {
        cmd.env("VEIL_PROFILE", prof);
    }
    if let Some(st) = opts.state {
        cmd.env("VEIL_STATE", st);
    }
    if let Some(p) = opts.provider {
        cmd.env("VEIL_PROVIDER", p);
    }
    if let Some(d) = opts.device {
        cmd.env("VEIL_IOS_DEVICE", d);
    }
    if let Some(sn) = opts.session_name {
        cmd.env("VEIL_SESSION_NAME", sn);
    }
    if let Some(dp) = opts.download_path {
        cmd.env("VEIL_DOWNLOAD_PATH", dp);
    }
    if let Some(ad) = opts.allowed_domains {
        cmd.env("VEIL_ALLOWED_DOMAINS", ad.join(","));
    }
    if let Some(ap) = opts.action_policy {
        cmd.env("VEIL_ACTION_POLICY", ap);
    }
    if let Some(ca) = opts.confirm_actions {
        cmd.env("VEIL_CONFIRM_ACTIONS", ca);
    }
    if let Some(engine) = opts.engine {
        cmd.env("VEIL_ENGINE", engine);
    }
    if opts.auto_connect {
        cmd.env("VEIL_AUTO_CONNECT", "1");
    }
    if let Some(idle) = opts.idle_timeout {
        cmd.env("VEIL_IDLE_TIMEOUT_MS", idle);
    }
    if let Some(cdp) = opts.cdp {
        cmd.env("VEIL_CDP", cdp);
    }
}
        ...
        if path_len > 103 {
            return Err(format!(
                "Session name '{}' is too long. Socket path would be {} bytes (max 103).\n\
                 Use a shorter session name or set VEIL_SOCKET_DIR to a shorter path.",
                session, path_len
            ));
        }
    }
...
mod tests {
    ...
    fn test_get_socket_dir_explicit_override() {
        let _guard = EnvGuard::new(&["VEIL_SOCKET_DIR", "XDG_RUNTIME_DIR"]);

        _guard.set("VEIL_SOCKET_DIR", "/custom/socket/path");
    ...
    fn test_get_socket_dir_xdg_runtime() {
        ...
        _guard.remove("VEIL_SOCKET_DIR");
        _guard.set("XDG_RUNTIME_DIR", "/run/user/1000");

        assert_eq!(
            get_socket_dir(),
            PathBuf::from("/run/user/1000/veil")
        );
    }
    ...
    fn test_get_socket_dir_home_fallback() {
        ...
        assert!(result.to_string_lossy().ends_with(".veil"));
        assert!(
