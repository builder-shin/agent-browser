...
fn get_traces_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".veil").join("tmp").join("traces")
    } else {
        std::env::temp_dir().join("veil").join("traces")
    }
}

fn get_profiles_dir() -> PathBuf {
    if let Some(home) = dirs::home_dir() {
        home.join(".veil").join("tmp").join("profiles")
    } else {
        std::env::temp_dir().join("veil").join("profiles")
    }
}
