...
/// Confirmation categories parsed from VEIL_CONFIRM_ACTIONS.
#[derive(Debug, Clone)]
...
impl ConfirmActions {
    pub fn from_env() -> Option<Self> {
        let val = env::var("VEIL_CONFIRM_ACTIONS").ok()?;
        if val.is_empty() {
            return None;
        }
...
impl ActionPolicy {
    ...
    /// Load policy if VEIL_ACTION_POLICY env var is set.
    /// Falls back to VEIL_POLICY for backwards compatibility.
    pub fn load_if_exists() -> Option<Self> {
        let path = env::var("VEIL_ACTION_POLICY")
            .or_else(|_| env::var("VEIL_POLICY"))
            .ok()?;
        Self::load(&path).ok()
    }
...
mod tests {
    ...
    fn test_confirm_actions_from_env() {
        let _guard = EnvGuard::new(&["VEIL_CONFIRM_ACTIONS"]);
        _guard.set("VEIL_CONFIRM_ACTIONS", "navigate,click,fill");
